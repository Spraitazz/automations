...
#
# TODO: xvfb_display -> xvfb_display_id is an int that can only be set in controller.py
#       as it must be unique (can clash when have multiple automations)
#
class SkelbiuAutomation(WebAutomation):
    ...
    def run_automation_cycle(self):
        """
        Run a single automation cycle: login, check and renew items.
        """
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            if self.check_need_renew():
                ...
                if not success:
                    self.logger.error(
                        "Automation cycle failed, sleeping for 60s before retry"
                    )
                    self.sleep(60.0)
                    continue
            else:
